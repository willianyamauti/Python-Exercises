...
def calculaMedias(matriz_consumo):
    vet_medias = []
    #verifica a quantidade de carros no vetor e gera a media
    for carro in range(len(matriz_consumo[0])): # como a quantidade de carros é igual para todos os pilotos,usei o len da primeira posição
        media = 0
        for linha in range(len(matriz_consumo)):
            media += matriz_consumo[linha][carro]
        vet_medias.append(media/len(matriz_consumo))# calcula a media do consumo
    return vet_medias
...
if not capacidadeTanques == '':
    ...
    if kmPorCondutores == '':
        ...
        if not compatibilidade_inputs(vet_capacidadeTanques, matriz_kmPorCondutores):
            ...
            media_matriz = calculaMedias(matriz_condutores)
            imprimeVetor(media_matriz)
else:
    print('E necessário pelo menos um automóvel')
